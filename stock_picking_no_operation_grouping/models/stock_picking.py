...
class StockPicking(models.Model):
    ...
    # Overwrite the whole method, as it's cant be overridden for this need
    # Add reservation ID to mapping and key
    def _prepare_pack_ops(self, quants, forced_qties):
        ...
        all_products = valid_quants.mapped('product_id') | self.env[
            ...
            p.id for p in forced_qties.keys()) | self.move_lines.mapped(
            'product_id')
        computed_putaway_locations = dict(
            (product, self.location_dest_id.get_putaway_strategy(
                product) or self.location_dest_id.id) for product in
            all_products)
        ...
        for move in picking_moves:
            ...
            if move.product_uom != product_to_uom[
                move.product_id.id] and move.product_uom.factor > \
                    product_to_uom[move.product_id.id].factor:
                product_to_uom[move.product_id.id] = move.product_uom
        if len(picking_moves.mapped('location_id')) > 1:
            raise UserError(_(
                'The source location must be the same '
                'for all the moves of the picking.'))
        if len(picking_moves.mapped('location_dest_id')) > 1:
            raise UserError(_(
                'The destination location must be the same '
                'for all the moves of the picking.'))

        pack_operation_values = []
        # find the packages we can move as a whole,
        # create pack operations and mark related quants as done
        ...
        for product, qty in forced_qties.items():
            ...
            key = _Mapping(product, self.env['stock.quant.package'],
                           self.owner_id, self.location_id,
        ...
        for mapping, qty in qtys_grouped.items():
            ...
            val_dict = {
                'picking_id': self.id,
                'product_qty': mapping.product.uom_id._compute_quantity(qty,
                                                                        uom),
            ...
            product_id_to_vals.setdefault(mapping.product.id, list()).append(
                val_dict)

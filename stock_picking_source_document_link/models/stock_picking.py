...
class StockPicking(models.Model):
    ...
    def _compute_source_document_link(self):
        ...
        for pick in self:
            ...
            pick.source_document_link_sale_order = False
            pick.source_document_link_purchase_order = False
            pick.source_document_link_stock_picking = False

            if origin.startswith("SO"):
                pick.source_document_link_sale_order = get_model_by_name_or_false(
                    "sale.order", origin
                )
            elif origin.startswith("PO"):
                pick.source_document_link_purchase_order = get_model_by_name_or_false(
                    "purchase.order", origin
                )
            elif origin.startswith("Return of WH"):
                pick.source_document_link_stock_picking = get_model_by_name_or_false(
                    "stock.picking",

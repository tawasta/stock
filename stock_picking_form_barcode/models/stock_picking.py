...
class StockPicking(models.Model):
    ...
    barcode_url = fields.Char(string="Barcode URL", compute="_compute_barcode_url")

    def _compute_barcode(self):
        for record in self:
            if isinstance(record.barcode_url, str) and len(record.barcode_url) > 0:
                img = urllib.request.urlopen(record.barcode_url).read()
                record.barcode = base64.b64encode(img)

    def _compute_barcode_url(self, code="EAN13"):
        ...
        for record in self:
            if isinstance(record.name, str) and len(record.name) > 0:
                ...
                record.barcode_url = record.barcode_url.replace("https:/", "https://")
                record.barcode_url = record.barcode_url.replace("http:/", "http://")
            else:
                record.barcode__url = ""
